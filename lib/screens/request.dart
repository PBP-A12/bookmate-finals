  ...
  Future<List<BookRequest>> fetchAllRequest(CookieRequest request, String sort) async {
    ...
    // Perform error handling for the response
      if (response.statusCode == 200) {
        ...
        if (data.length == 0) {
          return [BookRequest(id: 0, subjects:["N/A"], member: "N/A", title: "N/A", author: "N/A", year: 0, language: "N/A", dateRequested: DateTime.now())]; // Return an empty list when no requests have been made
        } else {
        ...
        if (sort == "title") {
          ...
          allRequestList.sort((a, b) => b.subjects.join().compareTo(a.subjects.join()));
        } else if (sort == "date requested") {
          allRequestList.sort((a, b) => b.dateRequested.compareTo(a.dateRequested));
        }
    ...
    // Perform error handling for the response
      if (response.statusCode == 200) {
        ...
        // print(userRequestList.toString());
          ...
          userRequestList.sort((a, b) => a.subjects.join().compareTo(b.subjects.join()));
        } else if (sort == "date requested") {
          userRequestList.sort((a, b) => a.dateRequested.compareTo(b.dateRequested));
        }
...
Widget build(BuildContext context) {
  ...
  const List<String> sort = <String>["Sort By", "Title", "Author", "Year", "Subjects", "Date Requested"];
            ...
            Row(
              ...
              children: [
                GestureDetector(
                  ...
                  child: Text('My Requests', style: GoogleFonts.lato(textStyle: TextStyle(color: Colors.black, fontSize: 28, fontWeight: FontWeight.bold, decoration: TextDecoration.underline, fontFamily: "Poppins"))),
                ),
                SizedBox(width: 30.0),
                GestureDetector(
                  onTap: () {
                  ...
                  child: Text('All Requests', style: GoogleFonts.lato(textStyle: TextStyle(color: Colors.black, fontSize: 28, fontWeight: FontWeight.bold, fontFamily: "Poppins", decoration: TextDecoration.underline))
                ...
                children: [
                  DropdownMenu<String>(
                    ...
                    onSelected: (value) {
                      ...
                      if (current == "user") {
                        _future = fetchUserRequest(request, value.toString().toLowerCase());
                      } else {
                        _future = fetchAllRequest(request, value.toString().toLowerCase());
                      }
                      setState(() {
            ...
            Expanded(
              child: FutureBuilder<List<BookRequest>>(
                ...
                builder: (context, snapshot) {
                  ...
                  } else if (snapshot.hasData && current == "user" && snapshot.data![0].id != 0) {
                    if (sortBy == "title") {
                      snapshot.data!.sort((a, b) => a.title.compareTo(b.title));
                    } else if (sortBy == "author") {
                      snapshot.data!.sort((a, b) => a.author.compareTo(b.author));
                      print(snapshot.data![0].title);
                    } else if (sortBy == "year") {
                      snapshot.data!.sort((a, b) => a.year.compareTo(b.year));
                    } else if (sortBy == "subjects") {
                      snapshot.data!.sort((a, b) => a.subjects.join().compareTo(b.subjects.join()));
                    } else if (sortBy == "date requested") {
                      snapshot.data!.sort((a, b) => a.dateRequested.compareTo(b.dateRequested));
                    }
                          ...
                          child: ListTile(
                                ...
                                Row(
                                  ...
                                  children: [
                                    ElevatedButton(
                                      style: ButtonStyle(
                                        backgroundColor: MaterialStateProperty.all<Color>(Color(0xFFC44B6A)),
                                        shape: MaterialStateProperty.all<RoundedRectangleBorder>(
                                          RoundedRectangleBorder(
                                            borderRadius: BorderRadius.circular(18.0),
                                            side: BorderSide(color: Color(0xFFC44B6A))
                                          )
                                        )
                                      ),
                                      onPressed: () async {
                                            ...
                                            onPressed: () async {
                                              ...
                                              await showDialog<void>(
                                                ...
                                                builder: (context) => AlertDialog(
                                                  ...
                                                  title: Text('Edit Requested Book'),
                                                      content: Stack(
                                                        clipBehavior: Clip.none,
                                                        children: <Widget>[
                                                          Container(
                  ...
                  } else if (snapshot.hasData && current == "all" && snapshot.data![0].id != 0) {
                    return ListView.builder(
                      itemCount: snapshot.data!.length,
                      itemBuilder: (context, index) {
                      String test = snapshot.data![index].dateRequested.toString();
